...
public partial class Game1
{
    ...
    private void DrawDeviceTabs()
    {
        ...
        for (var i = 0; i < items.Length; i++)
        {
            ...
            // Direct user bug report (screenshot) - a long catalog label (e.g. compartment entries
            // like "Реакторный отсек (тип 4, со срезанными углами)") measured far wider than one
            // item's own cell at the fixed DeviceItemFontScale, overflowing into neighboring cells'
            // labels and reading as unreadable overlapping text. Shrinks the scale per-item so its
            // own label always fits within its own cell (with a small margin) - a short label (every
            // existing device/tool name) already fits at the normal scale, so this never shrinks
            // those at all; only a label wider than the cell scales down.
            const float labelMargin = 6f;
            var rawWidth = _font.MeasureString(item.Label).X;
            var scale = rawWidth > 0f
                ? MathF.Min(DeviceItemFontScale, (DeviceItemCellWidth - labelMargin * 2) / rawWidth)
                : DeviceItemFontScale;
            var textSize = _font.MeasureString(item.Label) * scale;
            _spriteBatch.DrawString(_font, item.Label, new Vector2(rect.X + (rect.Width - textSize.X) / 2, rect.Y),
                DeviceGold, 0f, Vector2.Zero, scale, SpriteEffects.None, 0f);
        }
