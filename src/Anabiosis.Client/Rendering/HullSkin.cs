...
public static partial class HullSkin
{
    ...
    // Whether some OTHER room's rectangle sits anywhere in the diagonal quadrant just beyond this
    // corner, within a few tiles - not necessarily touching at exactly this point (that's
    // FindDiagonalNeighbor's own, stricter job, kept separate since DrawReentrantChamfers needs the
    // exact-touch neighbour's own rectangle to bridge two plates into one diagonal). Direct user
    // request: a corner shouldn't cosmetically read as "open to space" when there's clearly another
    // compartment sitting just past it, even if a genuine 1-2 tile gap (a corridor, a stray wall
    // margin) keeps it from touching at the exact point. Tolerance is a tunable heuristic, not a
    // hard rule - generous enough to catch "obviously right there" placements, not so large that a
    // compartment on the other side of the ship blocks a chamfer that should still read as open.
    private const float NearbyDiagonalToleranceUnits = 2f;

    private static bool HasNearbyDiagonalNeighbor(Room room, IReadOnlyList<Room> rooms, int corner)
    {
        foreach (var other in rooms)
        {
            if (ReferenceEquals(other, room))
                continue;
            var nearby = corner switch
            {
                0 => other.Right <= room.Left && other.Right > room.Left - NearbyDiagonalToleranceUnits
                     && other.Bottom <= room.Top && other.Bottom > room.Top - NearbyDiagonalToleranceUnits,
                1 => other.Left >= room.Right && other.Left < room.Right + NearbyDiagonalToleranceUnits
                     && other.Bottom <= room.Top && other.Bottom > room.Top - NearbyDiagonalToleranceUnits,
                2 => other.Left >= room.Right && other.Left < room.Right + NearbyDiagonalToleranceUnits
                     && other.Top >= room.Bottom && other.Top < room.Bottom + NearbyDiagonalToleranceUnits,
                _ => other.Right <= room.Left && other.Right > room.Left - NearbyDiagonalToleranceUnits
                     && other.Top >= room.Bottom && other.Top < room.Bottom + NearbyDiagonalToleranceUnits,
            };
            if (nearby)
                return true;
    ...
    // one end and interior at the other. HasNearbyDiagonalNeighbor (direct user request) additionally
    // keeps a corner square whenever another compartment is obviously sitting just past it, even
    // without an exact touching point.
    private static bool ShouldCutCorner(Room room, IReadOnlyList<Room> rooms, int corner)
    {
        if (FindDiagonalNeighbor(room, rooms, corner) is not null)
            return false;
        if (HasNearbyDiagonalNeighbor(room, rooms, corner))
            return false;
