...
//
// Arrow keys stay a hardcoded, always-on SECOND way to move (ReadMoveInput/ReadAimDirection/
// ReadHelmInput each still check them directly, in addition to whatever this rebinds) - preserves
// today's behavior for anyone already used to arrows, without needing 4 more rebindable rows for
// a second control scheme nobody asked to remap.
...
public sealed class PlayerActionBindings
{
    ...
    private static readonly Dictionary<PlayerAction, Keys> Defaults = new()
    {
        ...
        [PlayerAction.Interact] = Keys.E,
        [PlayerAction.Fire] = Keys.Space,
        [PlayerAction.HelmStabilize] = Keys.S,
        [PlayerAction.HelmReverseThrottle] = Keys.X,
        [PlayerAction.ToggleHelmMode] = Keys.Z,
    ...
    private static readonly Dictionary<PlayerAction, string> Labels = new()
    {
        ...
        [PlayerAction.HelmStabilize] = "Штурвал — стабилизация",
        [PlayerAction.HelmReverseThrottle] = "Штурвал — реверс тяги",
        [PlayerAction.ToggleHelmMode] = "Штурвал — режим управления",
