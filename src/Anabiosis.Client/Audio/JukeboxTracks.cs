...
public static class JukeboxTracks
{
    ...
    // Asset names are paths under Content/Music/Jukebox, without extension. The NN in a filename is
    // the order the track was imported in and nothing else - this array's order is the running order
    // the "Композиция" stepper walks through, and the two stopped matching once the list was
    // reordered by ear. Go by position here, not by the number in the name.
