...
public partial class Game1
{
    private void DrawMenuLightMask(float totalSeconds)
    {
        ...
        // no pool at all - direct user report ("корабль почти не видно"), then a follow-up ("видно
        // четче") after one big soft blob made it brighter but not any crisper: a single wide pool
        // brightens the black space around the hull by the same amount as the hull itself, so
        // contrast - and with it ScenePost's relief/specular read of the hull's own shape - barely
        // improves. Three tighter pools strung along the bake's own tail-to-nose line (same
        // pane-fraction coordinates DrawMenuEnginePlume's own NozzleMouths use) hug the actual
        // silhouette instead, so the light gradient breaks sharply at the hull's edge rather than
        // fading evenly across open space.
        Pool(blob, new Vector2(pane.X + pane.Width * 0.14f, pane.Y + pane.Height * 0.58f), pane.Width * 0.17f, new Color(200, 210, 230), 0.65f);
        Pool(blob, new Vector2(pane.X + pane.Width * 0.31f, pane.Y + pane.Height * 0.41f), pane.Width * 0.19f, new Color(200, 210, 230), 0.68f);
        Pool(blob, new Vector2(pane.X + pane.Width * 0.48f, pane.Y + pane.Height * 0.24f), pane.Width * 0.16f, new Color(210, 218, 236), 0.65f);
